...
$stmt->execute();
$don_hang = $stmt->fetchAll();
$connection = new PDO("mysql:host=127.0.0.1;dbname=123;chrset=utf8", "root");
$query = "select tong_tien,ngay_dathang from donhang where trang_thai = 1";
$stmt = $connection->prepare($query);
$stmt->execute();
$dh_da_giao = $stmt->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<style>
                                        ...
                                        <tbody>
                                            <?php foreach ($don_hang as $item) : ?>
                                                <tr>
                                                    ...
                                                    <td><?php echo $item['ngay_dathang'] ?></td>
                                                    <td><?php echo $item['dia_chi_gh'] ?></td>
                                                    <td><?php echo $item['ghi_chu'] ?></td>
                                                    <td><?php if ($item['trang_thai'] == 0) {
                                                            echo "Đang giao...";
                                                        }
                                                        if ($item['trang_thai'] == 1) {
                                                            echo "Đã giao hàng";
                                                        }  ?></td>
                                                    <td><a class="btn btn-primary " href="/duan1/DUAN12/DUAN1/chi-tiet-san-pham.php?id=<?php echo $item['ma_sp']; ?>">Xem Chi tiết</a></td>
                                                    <td><a class="btn btn-primary " href="delete-donhang.php?id=<?php echo $item['ma_dh']; ?>">Hủy Đơn</a></td>
                                                    <td><a class="btn btn-danger" href="dagiao.php?id=<?php echo $item['ma_dh'] ?>">Đã giao</a></td>
                                                </tr>
                                            <?php endforeach ?>
                                        </tbody>
                                    </table>
                                    <h3>Tổng số tiền hàng đã bán <?php $j = 0;
                                                                for ($i = 0; $i < count($dh_da_giao); $i++) {
                                                                    
                                                                    $j += $dh_da_giao[$i]["tong_tien"];
                                                                }
                                                                echo $j; ?>VNĐ</h3>
                                </div>
